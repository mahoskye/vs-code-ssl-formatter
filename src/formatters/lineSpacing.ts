...
interface ContentBlock {
    ...
    followingSpaces: number; // Type of content this block represents
...
function createContentBlocks(text: string): ContentBlock[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (patterns.comments.blockComment.test(line.trimmedContent)) {
            ...
            blocks.push({
                lines: [...currentBlock],
                followingSpaces: 0, // will be determined later
...
function formatBlocks(blocks: ContentBlock[]): string {
    // TODO: Implement formatting and joining
    return "";
}
