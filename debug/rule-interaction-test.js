/**
 * Debug script for testing the interaction between indentation and colon spacing rules
 */
...
// ColonSpacingRule class
class ColonSpacingRule {
    constructor() {
        this.name = "Colon Spacing";
        this.description = "Ensures proper spacing after colons in SSL keywords";
    }

    apply(line, context) {
        console.log(`ColonSpacingRule - Input: "${line}"`);
        ...
        if (trimmedLine.startsWith(":")) {
            ...
            if (keywordMatch) {
                ...
                const result = `${indentString}:${keyword.toUpperCase()}${rest ? " " + rest : ""}`;
                console.log(`ColonSpacingRule - Result: "${result}" (indentString="${indentString}")`);
                return result;
            }
        }
        console.log(`ColonSpacingRule - No change, result: "${line}"`);
        return line;
    }
}

// Formatting function
function formatWithRules(text) {
...
// Run the test
console.log('\n=== Testing rule interaction ===');
const result = formatWithRules(testInput);
...
// Detailed comparison if there's a mismatch
if (expected !== result) {
...
// Proposal for fixed ColonSpacingRule implementation
console.log('\n=== Proposed Fix for ColonSpacingRule ===');
console.log(`
    ...
    apply(line: string, context: FormattingContext): string {
        ...
        if (trimmedLine.startsWith(":")) {
            ...
            if (keywordMatch) {
                ...
                // Use the existing indentation plus the keyword formatting
